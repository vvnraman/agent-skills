...
Prefer splitting content into separate pages by type. Allow mixed content only when a single page is clearly better for reader workflow; keep one primary type and keep secondary content brief.
...
Use constraints as defaults, not absolute bans. Prefer reader usefulness over strict taxonomy when trade-offs exist.
